...
fun Int.calculatePercentage(divisor: Int): Double? =
    if (divisor != 0) { String.format("%.1f", 100.0 * this.div(divisor.toDouble())).toDouble()}
    else { null }


fun String.convertToDouble(): Double? = this.split(" ").first().toDoubleOrNull()

fun String.convertStringToDate(): Date? =  SimpleDateFormat(Constant.DATE_FORMAT).parse(this)
